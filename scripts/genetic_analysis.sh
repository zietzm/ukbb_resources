...
function ldak_h2 {
	...
	ROOT=/data1/home/mnz2108
	BLDPATH=$ROOT/data_resources/ukbiobank/hapmap3_genotypes/bld.ldak.hapmap.gbr.tagging
	THINPATH=$ROOT/data_resources/ukbiobank/hapmap3_genotypes/ldak.thin.hapmap.gbr.tagging

	OUT_DIR=$ROOT/git/maxgcp-analysis/data/h2_results
	OUTPATH="$OUT_DIR"/$(basename "$GWAS_FILE")

	# Format results for LDAK
	(
		echo -e 'Predictor\tA1\tA2\tn\tZ' &&
			xsv select -d $'\t' ID,A1,REF,ALT,OBS_CT,T_STAT "$GWAS_FILE" |
			xsv fmt -t $'\t' |
				awk '{
                    if ($2 == $3)
                        A2=$4;
	...
	# Heritability using the BLD-LDAK model
	ldak5.2.linux \
		--summary "${GWAS_FILE}.summaries" \
		--tagfile $BLDPATH \
		--check-sums NO \
		--cutoff 0.01 \
		--sum-hers "${OUTPATH}_bld"

	# Heritability using the LDAK-THIN model
	ldak5.2.linux \
		--summary "${GWAS_FILE}.summaries" \
		--tagfile $THINPATH \
		--check-sums NO \
		--cutoff 0.01 \
		--sum-hers "${OUTPATH}_thin"
}

export -f ldak_h2

# find $GWAS_PATH -name "plink_white_british*.glm.linear" | parallel --lb --jobs 40 --progress -- "ldak_h2 {}"
...
function ldak_rg {
	...
	OUT_DIR=$ROOT/git/maxgcp-analysis/data/rg_results
	OUTPATH="$OUT_DIR"/$(basename "$GWAS1")_$(basename "$GWAS2")
	RES="${OUTPATH}.cors"
	echo $RES
	if test -f $RES; then
		exit 0
...
# python write_ldak_jobs.py

# parallel --jobs 32 --progress -- <rg_jobs.txt
